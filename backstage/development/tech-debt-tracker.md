...
| 2026-07-06 | TD-005 | Frontend visual QA blocked in sandbox | Local Next dev/build paths cannot bind worker ports in the restricted runner, so desktop/narrow screenshot QA could not run here | Run manual desktop and narrow viewport QA in an unrestricted local or CI browser environment before merge |
| 2026-07-06 | TD-006 | Agent style docs are read-only in loop sandbox | `.agents/style/ui-patterns.md` could not be updated with the new Button/StateNotice guidance from issue #28 | Mirror the implemented frontend primitive guidance into `.agents/style/ui-patterns.md` from a maintainer-writable path |
