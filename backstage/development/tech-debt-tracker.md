...
| 2026-07-06 | TD-008 | Concrete public-data adapters still pending | The shared adapter/cache/fixture boundary is implemented, but provider-specific geocoding, demographics, economics, local-context, news, company-context, and domain-quality adapters still need live normalization and fixture coverage | Add one provider adapter at a time behind `backend/app/integrations/public_data.py`, with cache-backed fixture tests for timeout, missing-key, schema-change, and no-data paths |
| 2026-07-06 | TD-009 | Full Celery worker module still pending | Lead intake now records worker dispatch metadata through a service seam, but the repository still lacks the broker-backed worker module for true asynchronous execution | Complete the approved worker infrastructure slice so `SIGNAL_AGENT_EXECUTION_MODE=worker` can enqueue and process runs through the broker |
