...
if grep -Eq '^REVIEW_STATUS:[[:space:]]*needs-fix' "$message_file"; then
  apply_pr_labels "$pr_number" "agent:needs-fix" "agent:merge-ready,ci:passing,review:codex-clear"
elif grep -Eq '^REVIEW_STATUS:[[:space:]]*human' "$message_file"; then
  apply_pr_labels "$pr_number" "review:human,agent:needs-human" "agent:merge-ready,review:codex-clear"
  ...
  if [[ "$needs_human" == "true" ]]; then
    apply_pr_labels "$pr_number" "review:codex-clear" "agent:merge-ready,agent:needs-fix,ci:failed"
  elif [[ "$backend_ok" == "true" && "$frontend_ok" == "true" ]]; then
    apply_pr_labels "$pr_number" "review:codex-clear,agent:merge-ready,ci:passing" "agent:reviewing,agent:needs-fix,ci:failed"
  else
    apply_pr_labels "$pr_number" "review:codex-clear,agent:reviewing" "agent:merge-ready,agent:needs-fix,ci:failed"
  fi
else
